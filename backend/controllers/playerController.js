...
const PlayerController = {
  ...
  async testSportsRadar(req, res) {
    try {
      const player = await playerService.fetchAndCachePlayer(
  ...
  async debugPlayer(req, res) {
    try {
      const { id } = req.params;
      const playerData = await playerService.debugPlayerStats(id);
  ...
  async fetchPlayerStats(req, res) {
    try {
      ...
      const playerWithStats = await playerService.fetchAndCachePlayerStats(
        player.externalId
      );
